...
with open('features2num_WALS.dat', 'rb') as f1:
    features2num_WALS = pickle.load(f1) 
with open('features2num_SSWL.dat', 'rb') as f2:
    features2num_SSWL = pickle.load(f2)     
    
features = list(features2num_WALS.keys()) + list(features2num_SSWL.keys())
feature = features[68]
...
with open('./bert_embeddings/English_embedding.dat', 'rb') as fp:
    ...
    key = SSWL_INFO['English'][feature]
    y_train = torch.ones(10000) *features2num_SSWL[feature][key]
for l in languages:
    if l not in ['English','Chinese', 'Persian'] and feature in list(SSWL_INFO[l]):
        filename = './bert_embeddings/'+l+'_embedding.dat'    
        with open(filename, 'rb') as fp:
            x0 = torch.tensor(pickle.load(fp)) 
            key = SSWL_INFO[l][feature]
            y0 = torch.ones(10000) *features2num_SSWL[feature][key]
...
input_size = 768
num_hidden = 100
output_size =  len(features2num_SSWL[feature].keys())
...
with open('./bert_embeddings/Chinese_embedding.dat', 'rb') as fp:
    ...
    key = SSWL_INFO['Chinese'][feature]
    y_test = np.ones(10000)*features2num_SSWL[feature][key]
